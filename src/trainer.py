...
PATH_TO_LABELS = '/experiments/jsato/models/research/object_detection/data/oid_v4_label_map.pbtxt'
...
class Trainer(object):
    ...
    def get_classes_name_and_scores(self, boxes, classes, scores, category_index, label_id_offset, curr_index, max_boxes_to_draw=20, min_score_thresh=.4): 
        # 'min_score_thresh' returns bigger than 40% precision
        display_str = {}
        ...
        for i in range(min(max_boxes_to_draw, boxes.shape[0])):
            if scores is None or scores[i] > min_score_thresh:
                if classes[i] in six.viewkeys(category_index):
                    display_str['token_position'] = '{' + str(curr_index) + ',' + str(i) + '}'
                    display_str['class'] = category_index[classes[i] + label_id_offset]['name']
                    display_str['score'] = '{}%'.format(int(100 * scores[i]))
                    print(display_str)


    def mask_out_image(self, img_boxes, img_feats, img_labels, img_scores):
        ...
        img_scores_np = np.array(img_scores)
        label_id_offset = 1
        ...
        images_index = [2, 3, 6, 7, 8, 10, 17, 18, 20, 27, 29, 31]

        for i in range(len(path_to_images)):
            image_np = np.array(Image.open(path_to_images[i]))
            image_np_with_detections = image_np.copy()

            vis_util.visualize_boxes_and_labels_on_image_array(
                  image_np_with_detections,
                  img_boxes_np[images_index[i]],
                  img_labels_np[images_index[i]] + label_id_offset,
                  img_scores_np[images_index[i]],
                  category_index,
            ...
            self.get_classes_name_and_scores(img_boxes_np[images_index[i]], img_labels_np[images_index[i]], img_scores_np[images_index[i]], category_index, label_id_offset, i)
            print()
    ...
    def generate_batch_vpara(self, lang1, lang2, name):
        ...
        if lang2 is not None:
            # vTLM
            _, (boxes, feats, labels, scores), (x1, len1), (x2, len2) = self.get_batch_vpara(name, lang1, lang2)
            x, lengths, positions, langs = concat_batches(
                x1, len1, lang1_id, x2, len2, lang2_id, params.pad_index,
                params.eos_index, reset_positions=True)
        ...
        return x, lengths, positions, langs, image_langs, boxes, feats, labels, scores, (None, None)
    ...
    def vlm_step(self, lang1, lang2, lambda_coeff, iter):
        ...
        x, lengths, positions, langs, \
            image_langs, img_boxes, img_feats, img_labels, img_scores, _ = \
        ...
        img_boxes, img_feats, img_y, img_pred_mask, img_mask_pos = self.mask_out_image(
            img_boxes, img_feats, img_labels, img_scores)

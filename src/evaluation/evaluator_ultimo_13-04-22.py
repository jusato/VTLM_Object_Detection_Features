...
class Evaluator(object):
    ...
    def convert_to_text_masking(self, batch, lengths, dico, params, rng):
        """
        Convert a batch of sentences to a list of text sentences.
        """
        batch = batch.cpu().numpy()
        ...
        # nlp = spacy.load('pt_core_news_sm')
        noun_x = np.full((slen, bs), 0) 
        pred_mask = np.full((slen, bs), 100.0)
        num_nouns = 0
        ...
        for j in range(bs):
            ...
            for token in sentence_list:
                if join == True and token[-1] != '@':
                    temp += token
                    temp = temp.replace(" ", "")
                    words.append(temp)
                    join = False
                    temp_pos.append(pos)
                    dico_noun[temp] = temp_pos
                    temp_pos = [] 
                elif join == True and token[-1] == '@':
                    temp += token.split('@')[0]
                    temp_pos.append(pos)
                elif token[-1] == '@':
                    temp = token.split('@')[0]
            ...
            #assert lengths[j] == len(sentence.split())

            doc = nlp(newSentence)
            for token in doc:
                if token.pos_ == 'NOUN' and token.text != '<' and token.text != '>' and token.text != 'm' and token.text != 'mm':
                    try:
                        posicoes = dico_noun[token.text]       # posicoes[i]: posicao na frase em que noun == true (vai de 0 a len(frase))
                    except:
                        try:
                            posicoes = dico_noun[token.text + '.']
                        except:
                            print("----------------------------------")
                            ...
                            print("----------ERRO NO DICO -----------")
                            continue
                    if type(posicoes) == int:
                        posicoes += 1
                        pred_mask[posicoes, j] = rng.random()
                        noun_x[posicoes, j] = 1
                        num_nouns += 1
                    else: # type(posicoes) == list
                        for i in range(len(posicoes)):
                            posicoes[i] +=1
                            pred_mask[posicoes[i], j] = rng.random()
                            noun_x[posicoes[i], j] = 1
                            num_nouns += 1
            
        return pred_mask, noun_x, num_nouns
    ...
    def mask_out(self, x, lengths, rng):
        ...
        to_predict, noun_x, num_nouns = self.convert_to_text_masking(x, lengths, self.dico, self.params, rng)

        porc_nouns = num_nouns / (slen * bs) # Percentage of nouns among total tokens
        if porc_nouns >= params.word_pred:
            porc = (1/porc_nouns) * params.word_pred 
            assert porc > 0
            to_predict = to_predict <= porc # Select 15% 
        # If there are not enough nouns, it takes all the nouns plus tokens that are not nouns
        elif porc_nouns >= 0:
            porc = params.word_pred - porc_nouns
            for i in range(0, slen):
                for j in range(0, bs):
                    if noun_x[i][j] == 1:
                        to_predict[i][j] = 0
                    else:
                        to_predict[i][j] = rng.random()
            assert porc > 0
            to_predict = to_predict <= porc
        else:
            to_predict = rng.rand(slen, bs) <= params.word_pred
            num_zero_nouns+=1
            print("Number of times there are no nouns:", num_zero_nouns)

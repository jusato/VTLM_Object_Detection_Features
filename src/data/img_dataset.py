...
def load_images(sentence_ids, feat_path, img_names, n_regions):
    ...
    for idx in sentence_ids:
        ...
        with open(f_name, "rb") as f:
            ...
            #if idx == sentence_ids[2]:
                #print(x['detection_classes'])
                #print("------------")
                #print(x['detection_boxes'])
                #print("------------")
                #print(x['detection_scores'])
                #print("------------")
            ...
            #if idx == sentence_ids[0] or idx == sentence_ids[1]:
                #print(img_labels)
                #print("------------")
    ...
    img_scores = torch.from_numpy(
        np.array(img_scores, dtype=img_scores[0].dtype))

    return img_boxes, img_feats, img_labels, img_scores
...
class DatasetWithRegions(Dataset):
    ...
    def get_batches_iterator(self, batches):
        for sentence_ids in batches:
            ...
            img_boxes, img_feats, img_labels, img_scores = self.load_images(sentence_ids)

            yield (sent, (img_boxes, img_feats, img_labels, img_scores), sentence_ids)
...
class ParallelDatasetWithRegions(ParallelDataset):
    ...
    def get_batches_iterator(self, batches):
        for sentence_ids in batches:
            ...
            yield (sent1, sent2, (img_boxes, img_feats, img_labels, img_scores), sentence_ids)
